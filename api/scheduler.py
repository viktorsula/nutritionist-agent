...
(тот же процесс, что и Telegram-webhook). Раз в минуту проверяем, что наступило.

Рассылка клиентам идёт через `reminders`/`reminder_occurrences` (миграции 013/015/016):
run_reminders — наступившие напоминания одним пакетом, run_reminder_followups — контроль
ответа (детект/догон/просрочка). Прежний путь v0 через таблицу `notification_schedule`
(run_due_notifications + business_rules/notification_rules.py) удалён как мёртвый (P2-4):
записи в неё не создавал никто — ни API, ни UI, — поэтому джоб каждую минуту опрашивал
заведомо пустую таблицу. Сама таблица оставлена в БД (пустая, безвредная); её удаление —
отдельная миграция и отдельное решение.
...
# Простановка статуса 'overdue' просроченным задачам (P2-6). Раз в час: due_date хранится
# со временем, но реакция на просрочку не срочная — гонять это каждую минуту незачем.
_last_overdue_check_at: Optional[datetime] = None
OVERDUE_CHECK_EVERY_MIN = 60
...
async def run_overdue_tasks() -> None:
    """
    Пометить просроченные задачи статусом 'overdue' (P2-6).

    Раньше статус существовал в схеме, но не проставлялся никогда: `get_overdue_tasks()`
    вычислял просрочку на лету и никем не вызывался — и клиент, и нутрициолог видели
    просроченные задачи как обычные 'pending'. Достаточно раза в час: due_date хранится
    с точностью до времени, но реакция на просрочку не срочная.
    """
    global _last_overdue_check_at

    now = datetime.utcnow()
    if _last_overdue_check_at and (now - _last_overdue_check_at) < timedelta(
        minutes=OVERDUE_CHECK_EVERY_MIN
    ):
        return
    _last_overdue_check_at = now

    from database import queries

    try:
        overdue = queries.get_overdue_tasks()
        updated = queries.mark_tasks_overdue([t["id"] for t in overdue if t.get("id")])
    except Exception as e:
        logger.warning(f"scheduler: простановка overdue не удалась: {e}")
        return

    if updated:
        logger.info(f"scheduler: задач помечено просроченными: {updated}")
