...
def _format_client_profile(state: Dict[str, Any]) -> str:
    ...
    # Аллергии и непереносимости — РАЗНЫЕ вещи (миграция 022, P1-13). Модель обязана
    # трактовать их по-разному, иначе она либо запретит лишнее, либо пропустит опасное.
    intolerances = profile.get("intolerances")
    if intolerances:
        lines.append(
            "- Пищевые непереносимости (НЕ аллергия): " + format_list(intolerances)
            + ". Это дозозависимо и зависит от вида продукта — например, при непереносимости "
              "лактозы козий или овечий сыр часто допустим, а коровье молоко нет. "
              "НЕ считай это абсолютным запретом и не решай сам: если сомневаешься, "
              "скажи клиенту, что уточнишь у нутрициолога."
        )
