...
- **Редактор планов/задач — РЕАЛИЗОВАНО** (в карточке клиента: `TaskEditor` + `PlanEditor`,
  создание под RLS; план — деактивация старого до вставки нового).
- **Дальше по Фазе 3:** редактор ЗОЖ-плана (wellness) через UI; вкладки «Аналитика» (дашборды)
  и «Настройки» (пороги/источники/промпты) на React. Затем smoke-тест и PR в main.
