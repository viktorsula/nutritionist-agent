...
**⚠️ E2E РАУНД 2 (2 июля) — КБЖУ + UX уведомлений** (ветка `fix/meal-kbju-and-alert-ux`, 323 passed):
(№3, главное) `log_meal` не считал КБЖУ (`total` было опционально) → пусто в графиках → **обязали оценивать
`total`** (разблокирует графики питания, follow-up C); (№1) `log_wellbeing(bad)` теперь говорит модели
«нутрициолог уже уведомлён (панель+Telegram)» → «уже сообщила», не «будет в курсе»; (№2) `_finalize` отдаёт
ответ модели как есть (убран чуждый хвост «📋 Обратите внимание…»). Telegram-пуш `bad_wellbeing` ✅ подтверждён
на проде.

**▶️ РЕДИЗАЙН КАБИНЕТА НУТРИЦИОЛОГА (ClientCard) — В ХОДУ.** PR A (модель доступа) ГОТОВ
(ветка `feat/access-model-simplify`, 340 passed): `access_status` убран из гейтов (модель 2 оси:
`client_status`+`payment_status`/`paid_until`); `paused` блокирует и веб-кабинет; **авто-блок по `paid_until`**;
`POST /clients/{id}/status` с аудитом. **▶️ Далее PR B (фронт):** редизайн ClientCard — события наверху (само
событие из payload, дата слева, ≤30% высоты); объединения (задачи+заметки, telegram+пароль); редактор статусов
(лейбл `completed`→«Поддержка»); «План»→«Рекомендации нутрициолога»; 6 графиков питания 2-в-ряд (нормы); «Динамика
анализов»→«Доп. сведения». **Отдельно позже:** «Задачи→ежедневные Telegram-напоминания» (recurrence+джоб+тёплый текст).
Детали — `docs/progress.md` (часть 8), память [[project_nutritionist_cabinet_redesign]].

**✅ C — ГРАФИКИ ПИТАНИЯ — ЗАВЕРШЕНА.** C1 бэкенд (`get_nutrition_daily` + `GET /nutrition/daily`) влит PR #54.
C2 фронт (ветка `feat/nutrition-charts-frontend`, tsc+vitest зелёные): `NutritionChart` — «Питание» (К/Б/Ж/У+сахар
в одном поле, 2 оси) + «Вода» с нормами; кабинет клиента (`MainPanel`) и `ClientCard` нутрициолога; данные с
бэкенд-эндпоинта. Показатели питания — фикс. набор (≠ лабораторные `tracked_lab_indicators`; пищевой `sugar_g` ≠ глюкоза крови).
**Опц. остаётся:** переименование «Каталог показателей анализов» → «Референсные значения анализов» (лейбл лаб-фичи).
**▶️ После merge C2:** проверить E2E (клиент логирует еду с КБЖУ → график «Питание»/«Вода» рисуется у клиента и нутрициолога).

**⚠️ E2E РАУНД 1 (2 июля) — 4 корневых фикса** (ветка `fix/orchestrator-e2e`, 310 passed):
(1) клиент не читал дневник → scope `diary`; (2) в окне «LLM-модели» нет оркестраторов → **миграция 012**
(⚠️ применить в Supabase!); (3) нутрициолог не видел субъективных жалоб → `conversation_summary` в аналитику;
(4) жалоба не попадала в «Алерты» → промпт: симптом → обяз. `log_wellbeing(bad)`. Детали — `docs/progress.md`
(2 июля, часть 3). После merge — прогнать E2E повторно (особенно: пожаловаться на симптом → проверить «Алерты»
и аналитику; спросить у клиента «что вчера ел» → должен ответить из дневника).
...
**▶️ Ф3 — В ХОДУ.** **Шаг 0 (наблюдаемость покрытия) — КОД ГОТОВ** (ветка `feat/coverage-observability`, 320 passed):
`agents/core/coverage.py` метит каждый ход (`orchestrator` / `graph_flag_off` / `graph_fallback`), пишет
лог `COVERAGE …` + эндпоинт `GET /nutritionist/coverage`. Теперь критерии чистки графа ИЗМЕРИМЫ.
- **Как проверять обкатку:** `GET /nutritionist/coverage` (или `grep COVERAGE` в логах Render) → нужно
  `graph_fallback == 0` за период, доля `orchestrator` → 100% по мере снятия белых списков.
- **Дальше по Ф3 (ПОСЛЕ обкатки, по критериям):** снять белые списки → **чистка графа** (для обеих ролей;
  домейн/персист/безопасность переиспользуются, не удаляются). **Async** — под реальную нагрузку, отдельно
  (при 1–20 клиентах преждевременно; per-id `Lock` уже даёт корректность).
