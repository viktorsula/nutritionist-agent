...
**▶️ C — ГРАФИКИ ПИТАНИЯ.** C1 (бэкенд) ГОТОВ (ветка `feat/nutrition-charts-backend`, 328 passed):
`queries.get_nutrition_daily` + `GET /nutrition/daily` (клиент — свои, нутрициолог — любого; `{series,targets}`).
Показатели питания — ФИКСИРОВАННЫЙ набор (универсальны); per-client выбор — это про ЛАБОРАТОРНЫЕ показатели
(`tracked_lab_indicators`, уже есть). Пищевой `sugar_g` ≠ глюкоза крови. **▶️ Далее C2 (фронт):** график «Питание»
(К/Б/Ж/У в одном поле, две оси ккал/граммы) + «Вода» (норма) в кабинете клиента и в `ClientCard`; Recharts.
Опц. отдельно: «Каталог показателей анализов» → «Референсные значения анализов».
