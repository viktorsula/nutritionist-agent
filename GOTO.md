...
## ▶️ НАЧАТЬ ОТСЮДА (после 3 июля) — Напоминания ГОТОВЫ (Ф1+Ф2+follow-up влиты): E2E → хвосты/Ф3

**Где мы:** задача «Напоминания клиенту с контролем ответа» ЗАКРЫТА ПОЛНОСТЬЮ — **Фаза 1 (#59) +
Фаза 2 (2A #60 / 2B #61 / 2C #62) + docs (#63) + follow-up каталог-в-промпт (#64) влиты в main**.
Миграции `013`/`014`/`015` ✅ применены. Спецификация — `docs/spec_reminders.md`. Клиенту сообщения =
«Напоминания»; нутрициолог управляет в блоке «Задачи и заметки». 388 passed (бэк) + tsc/19 vitest (фронт).

**Что работает end-to-end:**
- **Ф1:** `reminders`+`reminder_occurrences`; планировщик `run_reminders()` — **пакет** по (клиент,
  локальное время) одним сообщением, durable-дедуп; тёплый текст (`task_type='reminder'` + промпт
  `client/reminder_message`); API/фронт `ReminderEditor` в «Задачи и заметки».
- **Ф2 контур ответа** (`run_reminder_followups()`): напоминание с «нужен ответ = показатель» →
  **детект** (запись нужного типа после отправки → `answered`) → **догон** (профили кадэнса) →
  **«сдались»** (→ low-severity алерт в панель). Профили в `system_settings.reminder_cadence`.
- **Ф2 показатели:** каталог `client_profiles.controlled_metrics` + таблица `client_metrics`;
  инструменты `log_measurement`/`log_sleep` (сон — переход через полночь); фронт `MetricsCatalogEditor`
  (пресеты вес/талия/шея/бёдра/сон + произвольные) + пикер ожидаемого ответа в `ReminderEditor`.
- **Follow-up (#64):** каталог `controlled_metrics` подаётся в системный промпт оркестратора
  (`_format_controlled_metrics` даёт ТОЧНЫЕ ключи) → и ПРОИЗВОЛЬНЫЕ показатели логируются под ключом
  каталога → детект детерминирован (не только built-in).

**⏳ E2E за владельцем:** в кабинете → блок «Задачи и заметки»: (1) добавить показатель в «Контролируемые
показатели» (напр. Вес или произвольный «Пульс»); (2) создать напоминание «Прислать вес», 07:00, ежедневно,
«нужен ответ = Вес»; (3) проверить приход в Telegram (пакет по времени), закрытие по присланному весу,
повтор/алерт при молчании. Зависимость: детект надёжен на оркестраторе (Екатерина).

**▶️ ДАЛЬШЕ (на выбор):**
1. **Графики `client_metrics`** (сон/произвольные показатели) в кабинете — `get_client_metrics` уже есть;
   нужен эндпоинт + компонент (по образцу графиков питания).
2. **Хвосты кабинета:** история планов дропдауном в `PlanEditor`.
3. **Ф3 (оркестраторы):** мониторить `GET /nutritionist/coverage` → при `graph_fallback==0` и доле
   `orchestrator`→100% снять белые списки → чистка графа. Async — отдельно, под нагрузку.
4. Опц.: «Каталог показателей анализов» → «Референсные значения анализов» (лейбл лаб-фичи).

**⚠️ Блокер вне кода:** OpenAI 429 (квота) — RAG (советы клиента, vector-аналитика) молчит, пока не пополнить.
