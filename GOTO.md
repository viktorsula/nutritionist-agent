...
## ▶️ НАЧАТЬ ОТСЮДА (после 3 июля) — Напоминания: E2E Фазы 1 → Фаза 2

**Где мы:** реализована **Фаза 1 напоминаний клиенту** (ветка `feat/reminders-phase1`, коммит запушен).
Спецификация обеих фаз — `docs/spec_reminders.md`. Клиенту сообщения = «Напоминания»; нутрициолог
управляет ими в блоке кабинета «Задачи и заметки». 359 passed (бэк) + tsc/19 vitest (фронт).
**Миграция `013_reminders.sql` ✅ применена в Supabase.**

**Что готово в Ф1:** таблицы `reminders`+`reminder_occurrences` (RLS backend-only); планировщик
`run_reminders()` с **пакетной** отправкой по (клиент, локальное время) одним сообщением, durable-дедуп
через occurrences; тёплый текст пакета (`task_type='reminder'` Groq + промпт `client/reminder_message`,
фолбэк-шаблон); API `GET/POST/PATCH/DELETE /clients/{id}/reminders` (аудит); фронт `ReminderEditor` в
«Задачи и заметки». Галочка «нужен ответ» — задел Ф2.

**⏳ E2E за владельцем:** в кабинете создать напоминание (напр. «Прислать вес», 07:00, ежедневно) →
проверить приход в Telegram одним пакетом в заданное локальное время клиента. Несколько на одно время →
одно сообщение.

**▶️ ДАЛЬШЕ — Фаза 2 (контроль ответа + контролируемые показатели):**
1. Расширить `reminder_occurrences`: `status(sent|answered|expired|closed)`, `response_event_id`,
   `followups_sent`, `next_followup_at`. Поле `reminders.expected_response` (ключ показателя|text|none).
2. Профили кадэнса в `system_settings.reminder_cadence` (формула `min(окно_профиля, следующее_срабатывание)`).
3. Модель «контролируемый показатель»: каталог `client_profiles.controlled_metrics` (JSONB) + таблица
   `client_metrics` + обобщённый `log_measurement(key,value)` в intake/оркестраторе (kinds `measurement`,
   `sleep`); роутинг: physical→measurements (weight/neck/waist/hips уже есть), lab→lab_results,
   sleep→client_metrics, custom→client_metrics. **Сон** = категория `sleep` (переход через полночь).
4. Планировщик: пасс детекта ответа (событие нужного типа после `sent_at` → `answered`); пасс догона;
   пасс «сдались» (→ low-severity алерт). Редактор напоминаний: выбор ожидаемого показателя + добавление
   нового (динамические поля). Зависимость: точный детект надёжен на **оркестраторе** (Екатерина).

**Прочие хвосты (из прошлой сессии):** история планов дропдауном в `PlanEditor`; Ф3 (мониторить
`GET /nutritionist/coverage` → чистка графа при `graph_fallback==0`). **Блокер вне кода:** OpenAI 429 (RAG).
