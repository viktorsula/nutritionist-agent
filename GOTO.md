...
- **Per-client показатели анализов — РЕАЛИЗОВАНО** (JSONB в client_profiles, редактор
  нутрициолога «Показатели анализов», график рисует выбранное с нормой). Нужен ввод значений
  в `lab_results` (пока вручную/через БД; парсер PDF — позже).
- **Дальше:** кабинет нутрициолога (Фаза 3) — панель алертов, реестр, карточка клиента,
  ввод значений `lab_results`, запросы к агенту.
