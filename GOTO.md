...
**Обновлено:** 20 июня 2026 — React-фронт, кабинет клиента (Фаза 2) работает локально.
...
- **Открытая задача:** per-client выбор показателей анализов нутрициологом для графика
  динамики (предложен JSONB `client_profiles.tracked_lab_indicators`; решить и реализовать).
- **Дальше:** кабинет нутрициолога (Фаза 3) — панель алертов, реестр, карточка клиента,
  выбор показателей анализов, ввод значений `lab_results`.
