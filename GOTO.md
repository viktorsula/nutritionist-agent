...
## ▶️ НАЧАТЬ ОТСЮДА (после 3 июля) — Слайс 2.2 (фронт контрольного списка) + E2E контура

**Где мы:** напоминания Ф1+Ф2 ГОТОВЫ, история планов и редизайн кабинета v2 (бэкенд контроля) влиты —
**#59–#69 в main**. Миграции `013`/`014`/`015`/`016` применяются владельцем. Спецификация —
`docs/spec_reminders.md` (низ — раздел «Переработка v2»). 396 passed (бэк) + tsc/19 vitest (фронт).
**Все детали — память [[project_reminders_feature]] + `docs/progress.md`.**

**Что уже работает (бэкенд контроля, #67–#69):**
- Приёмы пищи с ДЕДЛАЙНАМИ (завтрак 12:00 / обед 17:00 / ужин 22:00) → нет отчёта → `meal_not_reported`
  **medium + пуш нутрициологу**; вес/сон/… → `reminder_unanswered` low/панель.
- Детект по типу: вес/талия/шея/бёдра/грудь → `measurements`; сон/custom → `client_metrics`;
  вода → `water_logged`; еда → `calories_logged` по `meal_type`; labs → `lab_results`; text → сообщение.
- Per-элемент кадэнс (`followup_after_hours`/`max_followups`); оживлён `no_response` (джоб).
- Сообщение собирается из брифа: рекомендации плана + сегодняшние запросы + **вплетение неотвеченного
  со вчера** («вчера не прислали воду») → LLM по промпту `client/reminder_message`.
- Вёрстка кабинета: Профиль+Доступ, Статусы+Отчёты объединены; «Задачи и заметки» во всю ширину;
  история планов дропдауном; старый TaskEditor убран.

**▶️ СЛЕДУЮЩЕЕ — Слайс 2.2 (фронт `ControlListEditor`):** объединить каталог показателей + напоминания
в ОДИН «контрольный список». Строка элемента: заголовок / время / регулярность (ежедневно / день недели /
дата) / «ждать ответ» / «повтор через (ч)» / «число повторов»; для еды — дедлайн; предзаполнение из
пресетов (вес/талия/шея/бёдра/**грудь**/сон/**вода**/**завтрак/обед/ужин**) + произвольная строка.
API (`reminders` + `controlled-metrics`) уже готовы; поля `followup_after_hours`/`max_followups`/
`response_deadline`/`expected_response` в контракте есть. Заменяет `MetricsCatalogEditor`+`ReminderEditor`.

**⏳ E2E за владельцем (после миграции 016):**
1. Напоминание с показателем: каталог → «нужен ответ = Вес» → Telegram (пакет), закрытие/повтор/алерт.
2. Приёмы пищи с дедлайнами: нет обеда к 17:00 → **пуш нутрициологу** «клиент не прислал обед».
3. Сообщение с брифом: рекомендации плана + вплетение вчерашнего неотвеченного.
Зависимость: детект надёжен на оркестраторе (Екатерина).

**⚠️ Правило владельца:** слой алертов/контроля НЕ трогать без согласования (сделан целенаправленно).

**Прочие кандидаты:** графики `client_metrics` (сон/произвольные) — `get_client_metrics` есть;
Ф3 (чистка графа по `coverage`). **Блокер вне кода:** OpenAI 429 (RAG молчит).

Детали — `docs/spec_reminders.md`, `docs/progress.md` (сессия 3 июля), память [[project_reminders_feature]],
[[project_phase3_coverage]].

---

## ▶️ (архив) 3 июля — E2E редизайна кабинета + продолжение Ф3
