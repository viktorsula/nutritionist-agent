...
## ▶️ НАЧАТЬ ОТСЮДА (после 3 июля) — Напоминания ГОТОВЫ (Ф1+Ф2 влиты): E2E → follow-up/хвосты

**Где мы:** задача «Напоминания клиенту с контролем ответа» ЗАВЕРШЕНА — **Фаза 1 (#59) + Фаза 2
(2A #60 / 2B #61 / 2C #62) влиты в main**. Миграции `013`/`014`/`015` ✅ применены. Спецификация —
`docs/spec_reminders.md`. Клиенту сообщения = «Напоминания»; нутрициолог управляет в блоке «Задачи и заметки».
386 passed (бэк) + tsc/19 vitest (фронт).
...
показатели» (напр. Вес); (2) создать напоминание «Прислать вес», 07:00, ежедневно, «нужен ответ = Вес»;
(3) проверить приход в Telegram (пакет по времени), закрытие по присланному весу, повтор/алерт при молчании.

**▶️ ДАЛЬШЕ (на выбор):**
1. **Follow-up напоминаний:** подать каталог `controlled_metrics` клиента в промпт оркестратора →
   детерминированный детект ПРОИЗВОЛЬНЫХ показателей (сейчас built-in ключи ок, custom — best-effort).
   Опц.: графики `client_metrics` (сон/произвольные) — `get_client_metrics` уже есть.
