...
## 4c. Сброс пароля — по КОДУ (а не по ссылке)
Одноразовую ссылку из письма часто «прокликивает» сканер Gmail до пользователя →
токен гаснет → сброс не срабатывает. Поэтому сброс сделан по 6–8-значному коду.

**Требование (Supabase):** Authentication → Emails → Templates → **Reset Password** →
добавить в тело письма код `{{ .Token }}` (рядом со ссылкой или вместо неё).

Фронт: «Забыли пароль?» → email → «Прислать код» (`resetPasswordForEmail`) → ввод кода
(`verifyOtp({ type: 'recovery' })`) → экран «Новый пароль» (`updateUser`). Код не зависит
от клика по ссылке и иммунен к почтовым сканерам.
