...
## Статус: В разработке → ПРОД на Render (починка LLM + опыт фото + централизация моделей задеплоены)
Последнее обновление: 30 июня 2026
Сессия: починка прод-LLM, опыт фото, централизация моделей в llm_config (PR #32–#37 влиты)
