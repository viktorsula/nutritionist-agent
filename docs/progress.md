...
Сессия: привязка Telegram (E2E), сброс пароля, UX/промпты, Фаза 2 turn-буфер (PR #21–#23, #25 влиты)
...
**Дальше (согласовано):** (1) протокол `IntakeRecord` v1 — строгая JSON-схема извлечённых данных
(extract→domain→present), ДО Фазы 3; (2) Фаза 3 (цель воды + алерты воды/еды + end-of-day + окна
приёмов + оживить no_response). E2E Фазы 2 в Telegram — гоняет Виктор.
