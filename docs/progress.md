...
Сессия: React-фронт — кабинет клиента (Фаза 2), запуск + фиксы по UX/данным/LLM
...
- **Открытая задача:** per-client выбор показателей анализов нутрициологом для графика динамики
  (сейчас график рисует все показатели с данными; `system_settings.lab_indicators_top` не used).
