...
-- трансграничная передача — последнее актуально, пока LEGAL-2 (локализация в ОАЭ) отложена
-- решением владельца на этап пилота), с фиксацией версии текста/даты/канала для доказуемости.
--
...
-- только при ПОЛНОМ согласии на все три пункта — частичное согласие не пишется вовсе
...
CREATE TABLE IF NOT EXISTS client_consents (
    ...
    CONSTRAINT client_consents_all_required CHECK (
        health_data AND telegram_channel AND cross_border_transfer
    )
);
...
-- сервер сам проверяет, что все три пункта true, и сам определяет текущую версию текста —
